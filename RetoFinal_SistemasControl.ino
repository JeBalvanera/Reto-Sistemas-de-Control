...
// Encoder
const int pinChannelA = 8;
const int pinChannelB = 9;
...
float computePID(float target, float actual_speed) {
  ...
  // 3. componente derivativo (sobre la medición para evitar derivative kick)
  float derivative = -(actual_speed - speed_prev) / Ts;
  float D = Kd * derivative;
  speed_prev = actual_speed;

  // 4. Límites de la integral
  // Margen disponible después de aplicar P, y si ya saturó solito el margen es cero.
  float i_max = U_MAX - P;
  float i_min = U_MIN - P;
...
void setup() {
  ...
  // interrupciones de hardware para encoder
  enableInterrupt(pinChannelA, callback_A, FALLING);
  enableInterrupt(pinChannelB, callback_B, FALLING);
